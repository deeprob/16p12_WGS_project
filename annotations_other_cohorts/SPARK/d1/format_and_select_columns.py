...
# annotations we want
cols = ['Family','Role','Sample','Chr','Pos','Ref','Alt','Qual', 'Func.refGene','ExonicFunc.refGene',
...
# Func.refGene=intronic;Gene.refGene=SAMD11;GeneDetail.refGene=.;ExonicFunc.refGene=.;AAChange.refGene=.;

# make all the columns
df['Family'] = df.Sample.apply(lambda s: sampmapp.loc[s, 'sfid'])
df['Role'] = df.Sample.apply(lambda s: sampmapp.loc[s, 'role'])
...
df['gnomAD'] = df.Info.apply(lambda s: get_info_field(s, 'AF'))
...
extra_annotations = ['LOEUF', 'OMIM_phenotype']
