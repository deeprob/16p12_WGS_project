...
infilename = 'SPARK_pilot1379ind.ColumbiaJointCall.filtered.txt'
sample_map_filename = '/data4/SPARK/Pilot/documentation/1379ind_masterTable_20181016.txt'
outfilename = 'SPARK_pilot1379ind.ColumbiaJointCall.filtered.select_annotations.txt'
...
df = pd.read_csv(infilename, sep='\t', header=None, low_memory=False)

# name original columns
df.columns = ['Chr', 'Pos', 'ID', 'Ref', 'Alt', 'Qual', 'Filter', 'Info', 'Format', 'GT_Info', 'SampleID']

# annotations we want
cols = ['Family','Sample','Chr','Pos','Ref','Alt','Qual', 'Mut_type','Func.refGene', 'ExonicFunc.refGene',
		'Gene.refGene', 'Num_genes', 'RVIS_%.refGene', 'pLI.refGene',
		'CADD', 'gnomAD', 'MPC', 'MutationTaster_pred', 'GERP++_RS', 'ASD_risk_genes_TADA_FDR0.3',
...
df['Gene.refGene'] = df['Gene.refGene'].apply(lambda s: s.split('-'))
...
df['CADD'] = df.Info.apply(lambda s: get_info_field(s, 'CADD13_PHRED'))
df['gnomAD'] = df.Info.apply(lambda s: get_info_field(s, 'gnomAD_genome_ALL'))
df['MPC'] = df.Info.apply(lambda s: get_info_field(s, 'MPC'))
df['MutationTaster_pred'] = df.Info.apply(lambda s: get_info_field(s, 'MutationTaster_pred'))
