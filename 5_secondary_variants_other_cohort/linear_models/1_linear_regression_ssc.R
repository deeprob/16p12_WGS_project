...
linear_regression = function(df, formula, outfilename) {
	...
	summ[, "R2"] = summary(mod)$r.squared	# add R2
...
for (cohort in cohorts) {
	...
	for (col in pheno_cols) {
	  ...
	  for (p in c('SCZ_PRS', 'Rare_Deleterious_SNVs', 'Genes DEL', 'Genes DUP', 'STRs exonic',
	              'Rare_Deleterious_SNVs_LOEUF', 'DELs LOEUF<0.35', 'DUPs LOEUF<0.35', 'STRs exonic LOEUF<0.35')) {
	    formula = glue('`{col}` ~  Sex + `{p}`')
	    outname = gsub("<", "", p)
	    outfilename = glue('statistics/{cohort}_{col}_{outname}.tsv')
	    linear_regression(df, formula, outfilename)
	  }
	}
}
