...
SAMPLE=`head -n $line bed_files/13_nejm.bed | tail -n1 | cut -f 7`
CHR=`head -n $line bed_files/13_nejm.bed | tail -n1 | cut -f 1`
START=`head -n $line bed_files/13_nejm.bed | tail -n1 | cut -f 2`
END=`head -n $line bed_files/13_nejm.bed | tail -n1 | cut -f 3`
TYPE=`head -n $line bed_files/13_nejm.bed | tail -n1 | cut -f 4 | sed 's/<//' | sed 's/>//'`
...
samplot plot \
	...
	-o 'nejm_plots/'$SAMPLE'_'$CHR'_'$START'_'$END'_'$TYPE'.png' \
	-c $CHR \
	-s $START \
	-e $END \
	-t $TYPE
