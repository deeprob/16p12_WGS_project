# Merge adjacent calls per individual for each caller
# Our method for joining calls is:
# Merge calls if they:
# 1. Overlap (any %)
...
vcf = pd.read_csv(sys.argv[1], sep = '\t', skiprows = exclude, names = ['Chr','Pos', 'ID', 'Ref', 'Alt', 'Qual', 'FILTER', 'INFO', 'FORMAT', 'Sample'])
...
def merge_overlap_gap(df):
	to_skip = []
	df2 = pd.DataFrame(columns = ['Chr', 'Pos', 'End', 'Alt', 'ID'])
	for idx, row in df.iterrows():
		...
		co1_rows = df[(df.Alt==type) & (df.Chr==chr) & (df.Pos<=start) & (df.End>=end) & (df.ID!=id)]
		if co1_rows.shape[0] > 0:
			continue
		# Another CNV is completely covered - skip other CNV
		co2_rows = df[(df.Alt==type) & (df.Chr==chr) & (start<=df.Pos) & (end>=df.End) & (df.ID!=id)]
		...
		io1_rows = df[(df.Alt==type) & (df.Chr==chr) & (start<=df.Pos) & (end>=df.Pos) & (end<=df.End) & (df.ID!=id)]
		...
		io2_rows = df[(df.Alt==type) & (df.Chr==chr) & (df.Pos<=start) & (df.End>=start) & (df.End<=end) & (df.ID!=id)]
		...
		# <---> <--->
		# Current CNV is first
		sg1_rows = df[(df.Alt==type) & (df.Chr==chr) & (df.Pos>end) & (df.Pos<end+50000) & ((df.Pos-end)<((df.End-start)*0.2)) & (df.ID!=id)]
		if sg1_rows.shape[0] > 0:
			...
			if sg1_rows.shape[0] > 0:
				...
				continue
		# Current CNV is second
		sg2_rows = df[(df.Alt==type) & (df.Chr==chr) & (start>df.End) & (start<df.End+50000) & ((start-df.End)<((end-df.Pos)*0.2)) & (df.ID!=id)]
...
while not df1.equals(df2):
	df2.to_csv('intermediate_files/2_adjacent_filter/'+str(sample)+'_'+str(counter)+'.txt', index = False, sep = '\t')
	print('Wrote '+str(sample)+'_'+str(counter)+'.txt')
	new_df = merge_overlap_gap(df2)
	counter+=1
	df1 = df2
	df2 = new_df

df2.to_csv('tables/2_adjacent_filter/'+str(sample)+'_cnvnator_final.bed', index = False, sep = '\t')
