...
echo -e 'Chr\tStart\tEnd\tType\tID\tLength\tSample' > bed_files/4_merge_files.bed

for i in {2..346}
do
	SAMPLE=`cat ../cnvnator_files.csv | head -n $i | tail -n1 | cut -f2 -d,`
	...
	then
		BED_FILE='bed_files/3_split_by_size/'$SAMPLE'_cnvnator_small.bed'
		tail -n+2 $BED_FILE >> bed_files/4_merge_files.bed
	else
