...
grep 'de novo' bed_files/16_inheritance.bed > bed_files/17_denovo_cnvs.bed

lines=`wc -l bed_files/17_denovo_cnvs.bed | cut -f 1 -d ' '`

# Make graphs for 29 de novo plots
for (( line=1; line<=$lines; line++ ));
do
	# Get information from CNV call
	SAMPLE=`head -n $line bed_files/17_denovo_cnvs.bed | tail -n1 | cut -f 7`
	CHR=`head -n $line bed_files/17_denovo_cnvs.bed | tail -n1 | cut -f 1`
	START=`head -n $line bed_files/17_denovo_cnvs.bed | tail -n1 | cut -f 2`
	END=`head -n $line bed_files/17_denovo_cnvs.bed | tail -n1 | cut -f 3`
	TYPE=`head -n $line bed_files/17_denovo_cnvs.bed | tail -n1 | cut -f 4 | sed 's/<//' | sed 's/>//'`
