...
# Make graphs for 20 random plots

# line will be a random number
line=$1

# Get information from CNV call
SAMPLE=`head -n $line bed_files/12_frequency_filter.bed | tail -n1 | cut -f 7`
CHR=`head -n $line bed_files/12_frequency_filter.bed | tail -n1 | cut -f 1`
START=`head -n $line bed_files/12_frequency_filter.bed | tail -n1 | cut -f 2`
END=`head -n $line bed_files/12_frequency_filter.bed | tail -n1 | cut -f 3`
TYPE=`head -n $line bed_files/12_frequency_filter.bed | tail -n1 | cut -f 4 | sed 's/<//' | sed 's/>//'`
