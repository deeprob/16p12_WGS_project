...
filename = '/data5/16p12_WGS/structural_variants/expansions_strs2/16p12_cohort.expansions_2SD.hg19_multianno.txt'
anno = pd.read_csv(filename, sep='\t')
anno['variant_id'] = anno['Chr'] + '_' + anno['Start'].astype(str) + '_' + anno['Alt']
anno = anno.set_index('variant_id')
anno = anno.drop_duplicates().copy()


filename = '/data5/16p12_WGS/structural_variants/expansions_strs/output/16p12_cohort.expansions_2SD.tsv'
...
df.to_csv('16p12_cohort.expansions_2SD.annotated.tsv', sep='\t', index=False)
