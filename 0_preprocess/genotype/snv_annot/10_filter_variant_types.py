...
df = pd.read_csv('exonic_variants/rare_exonic_variants.csv')
...
# I can filter out some cadd here
df = df[((df['CADD_PHRED'].isna()) | (df['CADD_PHRED'] >= 25))]
...
df.to_csv('exonic_variants/rare_deleterious_exonic_variants.csv', index=False)
