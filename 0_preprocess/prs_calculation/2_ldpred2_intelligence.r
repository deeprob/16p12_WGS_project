...
sumstats <- fread("../final_files/intelligence_final.txt.gz", fill=T) %>%
...
names(sumstats) <-c("rsid","uniq_id","chr","pos","a1","a0","EAF","Z","beta","beta_se","p","n_eff","INFO","EFFECT_DIR") #Change based on header
...
snp_readBed("../16p12_microarray_final_intelligence.bed")
# now attach the genotype object
obj.bigSNP <- snp_attach("../16p12_microarray_final_intelligence.rds")
...
prs_gemma<-t(prs_gemma)
prs_gemma<-cbind("intelligence_auto",0,0,prs_gemma)
results<-rbind(results, prs_gemma)

prs_auto<-t(pred_auto)
prs_auto<-cbind("intelligence_auto_noscale",0,0,prs_auto)
...
write.table(results,file="prs_gemma_intelligence.bimbam",sep="\t",row.names=FALSE,col.names=FALSE,quote=FALSE)
