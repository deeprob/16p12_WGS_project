...
sumstats <- fread("../final_files/schizophrenia_final.txt.gz") %>%
    setnames(., colnames(.), c("CHR", "SNP", "A1", "A2", "BP", "INFO", "OR", "SE", "P", "NGT")) %>% 
     .[,c("CHR"):=list(sub('...', '', CHR))] %>% .[,c("CHR"):=list(strtoi(CHR))]  
names(sumstats) <-c("chr","rsid","a0","a1","pos","INFO","OR","SE","p","ngt") #Change based on header
...
sumstats$n_eff <- 4 / (1/36989 + 1/113075)
...
snp_readBed("../16p12_microarray_final_schizophrenia.bed")
# now attach the genotype object
obj.bigSNP <- snp_attach("../16p12_microarray_final_schizophrenia.rds")
...
prs_gemma<-t(prs_gemma)
prs_gemma<-cbind("SCZ_auto",0,0,prs_gemma)
results<-rbind(results, prs_gemma)

prs_auto<-t(pred_auto)
prs_auto<-cbind("SCZ_auto_noscale",0,0,prs_auto)
...
write.table(results,file="prs_gemma_SCZ.bimbam",sep="\t",row.names=FALSE,col.names=FALSE,quote=FALSE)
