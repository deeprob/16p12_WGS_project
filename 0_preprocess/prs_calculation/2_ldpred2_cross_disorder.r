...
sumstats <- fread("../final_files/cross_disorder_final.txt.gz", fill=T) %>%
...
names(sumstats) <-c("chr","pos","rsid","a0","a1","beta","beta_se","p","NGT","FCAS","FCON","IMPINFO","NEFFDIV2","NCAS","NCON","DIRE","ASSET","m.SCZ","m.BIP","m.MD","m.ASD","m.ADHD","m.TS","m.AN","m.OCD") #Change based on header
sumstats <- sumstats[sumstats$rsid%in% info$rsid,] #Filter out SNPs not in HapMap3 dataset
#Calculate effective sample size from literature if not present in summary statistics
sumstats$n_eff <- (232964+494162)
...
snp_readBed("../16p12_microarray_final_cross_disorder.bed")
# now attach the genotype object
obj.bigSNP <- snp_attach("../16p12_microarray_final_cross_disorder.rds")
...
prs_gemma<-t(prs_gemma)
prs_gemma<-cbind("cross_disorder_auto",0,0,prs_gemma)
results<-rbind(results, prs_gemma)

prs_auto<-t(pred_auto)
prs_auto<-cbind("cross_disorder_auto_noscale",0,0,prs_auto)
...
write.table(results,file="prs_gemma_cross_disorder.bimbam",sep="\t",row.names=FALSE,col.names=FALSE,quote=FALSE)

keep_snps<-!is.infinite(rowSums(beta_auto))
test <- beta_auto[keep_snps,]
test2 <- info_snp$`_NUM_ID_`[keep_snps]
pred_auto <-big_prodMat(genotype,test,ind.row = ind.test,ind.col = test2)
