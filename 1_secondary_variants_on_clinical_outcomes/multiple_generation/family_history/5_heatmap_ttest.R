...
df = read.csv('statistics/3_ttest_family_history.csv', check.names=F)

phenotype_groups = df$Phenotype
phenotype_groups <- phenotype_groups[!duplicated(phenotype_groups)]

# skip splice cadd louef because one of the ttests came out with NA for that test
variant_groups = c('Missense_CADD25', 'Missense_CADD25_LOEUF035', 'LOF', 'LOF_LOEUF_035', 'Splice_CADD25', 'genes_del', 'genes_dup', 'dels_loeuf', 'dups_loeuf', 'STRs_exonic', 'STRs_exonic_LOEUF035', 'Rare_Deleterious_SNVs', 'Rare_Deleterious_SNVs_LOEUF', 'enhancer', 'promoter', 'UTR5', 'intelligence_PRS', 'SCZ_PRS', 'educational_attainment_PRS', 'autism_PRS')
...
pdf(glue('figures/3_ttest_family_history.pdf'))

# there is one na in the dataframe
# remove to make the figure
dfplot = df
min_num_samples = min(dfplot$Num_samples)
max_num_samples = max(dfplot$Num_samples)
title_text = glue('Cohen\'s D ({min_num_samples}-{max_num_samples} Samples)')
p_mat = make_square_matrix(dfplot, 'Pvalue')
cor_mat = make_square_matrix(dfplot, 'Cohen_D')
...
cols <- cols[grepl('phenotype_parents', cols)]

corrplot(
  cor_mat[, cols],
  p.mat = fake_p_mat[, cols], 
  method = "color",
  insig = "label_sig",
  sig.level = c(.001, .01, .05),
  pch.cex = .75,
