...
scrna_df = read.csv('allen_institute_tables/cell_type_specific_expression.csv', check.names=F)
...
for (variant_class in variant_classes) {
	app = read.csv(glue('{dropbox}/16p12.2 project/Human patients project/WGS paper/19_Variant_GO_enrichment/figure_for_16p12_cohort/variants/{variant_class}.csv'))
...
master_df = read.csv(glue('{dropbox}//16p12.2 project/Human patients project/WGS paper/11_Variant Integration/16p12_cohort_summary_v18.csv'), check.names=F)
...
gencode_df = read.csv(glue('{dropbox}/16p12.2 project/Human patients project/WGS paper/5_SNV pipelines-annotations/GENCODE annotations/gencode.v19.parsed.genes.csv'), check.names=F)
...
write.csv(stats, 'statistics/fishers_exact_with_R.csv', row.names=F)
