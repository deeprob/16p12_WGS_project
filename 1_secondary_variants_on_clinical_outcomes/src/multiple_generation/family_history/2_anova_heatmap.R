...
df = read.csv('statistics/anova_family_history.csv', check.names=F)
...
make_square_matrix = function(df, column) {
	...
	for (col1 in variant_groups) {
		for (col2 in phenotype_groups) {
			...
			value = df[(df$Variant == col1) & (df$Family_History == col2), column]
...
pdf(glue('figures/anova_family_history.pdf'))
