...
df = read.table('statistics/correlations_rare_variant_prs.tsv', sep='\t', header=T)
head(df)




df = reshape(df, idvar = c('PRS'), timevar = "Phenotype", direction = "wide")
head(df)


rownames(df) = df$PRS

cols = c('Child_ID_DD', 'Child_behav', 'Child_psych', 'Child_nervous_system', 'Child_congenital', 'Child_craniofacial')

# make p matrix and cohen's d matrix
p_mat = df[paste('pvalue.', cols, sep='')]
d_mat = df[paste('R.', cols, sep='')]
